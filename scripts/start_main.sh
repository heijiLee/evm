...
HOMEDIR="./evmd-local/.testnets/node0"
...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
	...
	# Set client config
	$EVMDBIN config set client chain-id "$CHAINID" --home "$HOMEDIR"
	$EVMDBIN config set client keyring-backend "$KEYRING" --home "$HOMEDIR"
  ...
  VAL3_KEY="mykey3"
	...
	echo "$VAL_MNEMONIC" | $EVMDBIN keys add "$VAL_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"
	echo "$USER1_MNEMONIC" | $EVMDBIN keys add "$USER1_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"
	echo "$USER2_MNEMONIC" | $EVMDBIN keys add "$USER2_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"
	echo "$USER3_MNEMONIC" | $EVMDBIN keys add "$USER3_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"
	echo "$USER4_MNEMONIC" | $EVMDBIN keys add "$USER4_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"

	# Set moniker and chain-id for the example chain (Moniker can be anything, chain-id must be an integer)
	$EVMDBIN init $MONIKER -o --chain-id "$CHAINID" --home "$HOMEDIR"
	...
	$EVMDBIN genesis add-genesis-account "$VAL_KEY" 100000000000000000000000000atest --keyring-backend "$KEYRING" --home "$HOMEDIR"
 	# evmd genesis add-genesis-account "$VAL1_KEY" 100000000000000000000000000atest --keyring-backend "$KEYRING" --home "$HOMEDIR"
	# evmd genesis add-genesis-account "$VAL2_KEY" 100000000000000000000000000atest --keyring-backend "$KEYRING" --home "$HOMEDIR"
	# evmd genesis add-genesis-account "$VAL3_KEY" 100000000000000000000000000atest --keyring-backend "$KEYRING" --home "$HOMEDIR"
 
	$EVMDBIN genesis add-genesis-account "$USER1_KEY" 1000000000000000000000atest --keyring-backend "$KEYRING" --home "$HOMEDIR"
	$EVMDBIN genesis add-genesis-account "$USER2_KEY" 1000000000000000000000atest --keyring-backend "$KEYRING" --home "$HOMEDIR"
	$EVMDBIN genesis add-genesis-account "$USER3_KEY" 1000000000000000000000atest --keyring-backend "$KEYRING" --home "$HOMEDIR"
	$EVMDBIN genesis add-genesis-account "$USER4_KEY" 1000000000000000000000atest --keyring-backend "$KEYRING" --home "$HOMEDIR"

	# Sign genesis transaction
	$EVMDBIN genesis gentx "$VAL_KEY" 1000000000000000000000atest --gas-prices ${BASEFEE}atest --keyring-backend "$KEYRING" --chain-id "$CHAINID" --home "$HOMEDIR"
	...
	# Collect genesis tx
	$EVMDBIN genesis collect-gentxs --home "$HOMEDIR"

	# Run this to ensure everything worked and that the genesis file is setup correctly
	$EVMDBIN genesis validate-genesis --home "$HOMEDIR"
